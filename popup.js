...
async function guess() {
    ...
    await pyodide.loadPackage([
        ...
        "pyodide_http-0.2.1-py3-none-any.whl", "pywidevine-1.8.0-py3-none-any.whl",
        "requests-2.31.0-py3-none-any.whl", "urllib3-2.2.1-py3-none-any.whl"
    ].map(e => "wheels/" + e));
...
// Get and display the last stored MPD URL
function autoMPD() {
...
// Save the current MPD URL to clipboard
function SaveMPD() {
...
// Add event listeners for MPD functionality
document.getElementById('MPDButton').addEventListener("click", SaveMPD);
autoMPD();  // Automatically fetch and display last MPD URL

// Add event listener for copying result content
document.getElementById('resultCopyButton').addEventListener('click', function () {
    ...
    if (resultText.value.length > 0) {
        ...
        navigator.clipboard.writeText(resultText.value).then(() => {
            ...
            console.error('Failed to copy Key(s):', err);
        });
    }
});
